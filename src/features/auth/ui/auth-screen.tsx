...
import { useRequestCodeMutation, useVerifyCodeMutation } from "@/features/auth/api";
...
import { ADAPTIVE_VIEWPORT_HEIGHT, SAFE_AREA_TOP, SAFE_AREA_BOTTOM } from "@/shared/lib/ui-styles";
...
function OtpCodeInput({
  ...
  return (
    <div className="space-y-2">
      <div className="mx-auto flex max-w-[320px] justify-center gap-2" role="group" aria-label="Код подтверждения">
        {digits.map((digit, idx) => (
          <input
            ...
            onPaste={(e) => {
              e.preventDefault();
              const target = firstEmptyIndex(digits);
              fillFrom(target === -1 ? idx : target, e.clipboardData.getData("text"));
            }}
            className={[
      ...
      {error ? (
        <p id="error-message" className="text-center text-sm text-red-500" role="alert">
          {error}
        </p>
      ) : null}
    </div>
...
export function AuthScreen() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setResendLeftSec((s) => Math.max(0, s - 1)), 1000);
  ...
  async function onRequestCode(e: FormEvent) {
    ...
    try {
      ...
      setError(appErrorText(err, "Сервис временно недоступен. Повторите через минуту."));
    }
  }
    ...
    >
      <Card variant="elevated" padding="lg" className="w-full">
        <CardHeader>
          ...
          <div className="mx-auto mb-4 relative flex items-center justify-center" style={{ width: '104px', height: '56px' }}>
          ...
          {step === 1 ? (
            <form onSubmit={onRequestCode} className="space-y-4">
              <Input
                ...
                value={reddyUserKey}
                onChange={(e) => setReddyUserKey(e.target.value.replace(/\D/g, "").slice(0, 32))}
                error={error}
                fullWidth
                required
            ...
            <form onSubmit={onVerifyCode} className="space-y-4">
              <OtpCodeInput value={otp} onChange={setOtp} error={error} autoFocus />
              ...
              >
                {resendLeftSec > 0 ? (
                  <span className="tabular-nums">Повтор через {resendLeftSec}с</span>
                ) : (
                  "Отправить код снова"
                )}
              </Button>
