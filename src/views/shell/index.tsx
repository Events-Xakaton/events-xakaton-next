...
const HomeScreen = lazy(() => import("@/views/home").then(m => ({ default: m.HomeScreen })));
const CreateScreen = lazy(() => import("@/views/create").then(m => ({ default: m.CreateScreen })));
const NotificationsScreen = lazy(() => import("@/views/notifications").then(m => ({ default: m.NotificationsScreen })));
const PointsScreen = lazy(() => import("@/views/points").then(m => ({ default: m.PointsScreen })));
const AccountScreen = lazy(() => import("@/views/account").then(m => ({ default: m.AccountScreen })));
const EventDetails = lazy(() => import("@/views/event-details").then(m => ({ default: m.EventDetails })));
const ClubDetails = lazy(() => import("@/views/club-details").then(m => ({ default: m.ClubDetails })));

export default function MiniAppShell() {
  const dispatch = useAppDispatch();
  const isVerified = useAppSelector((s) => s.auth.isVerified);
  const [tab, setTab] = useState<MainTab>("home");
  const [detail, setDetail] = useState<{ kind: "event" | "club"; id: string } | null>(null);
  ...
  return (
    ...
    >
      <div
        className={
          homeNoDetail || createNoDetail || notificationsNoDetail || accountNoDetail
            || pointsNoDetail
        ...
        style={
          !homeNoDetail && !createNoDetail && !notificationsNoDetail && !pointsNoDetail && !accountNoDetail && !inDetail
